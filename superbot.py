...
async def on_message(message):
    if message.content.startswith("!help"):
        ...
        embed.add_field(name='**!help**', value="shows this command with good gui :stuck_out_tongue:\n`Syntax: !help`\n")
        embed.add_field(name='**!life**', value="send a life through the Quiz Dog!\n`Syntax: !life <phone>`\n")
        embed.add_field(name='**!confirm**', value="confirm a life through the Quiz Dog.\n`Syntax: !confirm <code> <referral>`\n")
    ...
    if message.content.startswith("!life"):
        ...
        if phone == "":
            ...
            await bot.send_message(message.channel, f"<@%s>: Verification code sent to `{phone}`, please use `!confirm <code> <referral>` to queue the life and bork." % message.author.id)

    if message.content.startswith("!confirm"):
        args = message.content.split(" ")
        secondargs = (" ".join(args[1:]))
        if secondargs == "" or not len(args[1]) == 4:
            await bot.send_message(message.channel, "<@%s>: Invalid arguments. Use `!life <phone>` and then `!confirm <code> <referral>` to queue the life, you dont know what to do hooman." % message.author.id)
        else:
            code = args[1]
            referral = args[2]
