...
async def on_message(message):
    if message.content.startswith("!help"):
        embed = discord.Embed(title="Quiz Dog", description="<@%s>: Commands" % message.author.id, color=0x7289DA, icon='https://i.imgur.com/HxnbbIe.jpg')
        ...
        embed.add_field(name='**!confirm**', value="confirm a life through the mystical bot.\n`Syntax: !confirm <code> <referral>`\n")
        embed.set_footer(text="Quiz Dog | !help", icon_url='https://i.imgur.com/HxnbbIe.jpg')
    ...
    if message.content.startswith("!confirm"):
        ...
        if secondargs == "" or not len(args[1]) == 4:
            ...
            await bot.send_message(message.channel, f"<@%s>: Account created. Life has been applied to the account `{referral}` with super bot which is be abuse!." % message.author.id)
